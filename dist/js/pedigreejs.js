...
(function(io, $, undefined) {
  ...
  io.print = function(el) {
    ...
    var cssFiles = ['/static/css/output.css', 'https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css'];
    var printWindow = window.open('', 'PrintMap', 'width=' + width + ',height=' + height);
    var headContent = '';
    for (var i = 0; i < cssFiles.length; i++) headContent += '<link href="' + cssFiles[i] + '" rel="stylesheet" type="text/css" media="all">';
    ...
    for (i = 0; i < el.length; i++) {
      html += $(el[i]).html();
      if (i < el.length - 1) html += '<div style="page-break-before:always"> </div>';
    }
  ...
  io.load = function(e, opts) {
    ...
    if (f) {
      ...
      reader.onload = function(e) {
        ...
        try {
          if (e.target.result.startsWith("BOADICEA import pedigree file format 4.0")) opts.dataset = io.readBoadiceaV4(e.target.result);
          else {
            try {
  ...
  io.readLinkage = function(boadicea_lines) {
    ...
    for (var i = 0; i < lines.length; i++) {
      var attr = $.map(lines[i].trim().split(/\s+/), function(val, i) {
        return val.trim();
      });
  ...
  io.readBoadiceaV4 = function(boadicea_lines) {
    var lines = boadicea_lines.trim().split('\n');
    var ped = [];
    var cancer_check = function(cancer, diagnosis_age) {
    ...
    for (var i = 2; i < lines.length; i++) {
      ...
      if (attr.length > 1) {
        ...
        for (var j = 0; j < io.genetic_test.length; j++) {
          idx += 2;
          if (attr[idx - 2] !== '0') {
            if ((attr[idx - 2] === 'S' || attr[idx - 2] === 'T') && (attr[idx - 1] === 'P' || attr[idx - 1] === 'N')) indi[io.genetic_test[j] + '_gene_test'] = {
              'type': attr[idx - 2],
              'result': attr[idx - 1]
            };
        ...
        for (j = 0; j < io.pathology_tests.length; j++) {
          if (attr[idx] !== '0') {
            if (attr[idx] === 'N' || attr[idx] === 'P') indi[io.pathology_tests[j] + '_bc_pathology'] = attr[idx];
            else console.warn('UNRECOGNISED PATHOLOGY ON LINE ' + (i + 1) + ": " + io.pathology_tests[j] + " " + attr[idx]);
          }
          idx++;
        }
        ped.unshift(indi);
  ...
  function getPartnerIdx(dataset, anode) {
    ...
    for (var i = 0; i < dataset.length; i++) {
      var bnode = dataset[i];
      if (anode.name === bnode.mother) return pedigree_util.getIdxByName(dataset, bnode.father);
      else if (anode.name === bnode.father) return pedigree_util.getIdxByName(dataset, bnode.mother);
    }
    return -1;
  }
...
(function(pedigree_util, $, undefined) {
  pedigree_util.buildTree = function(opts, person, root, partnerLinks, id) {
    if (typeof person.children === typeof undefined) person.children = pedigree_util.getChildren(opts.dataset, person);
  ...
  function setChildrenId(children, id) {
    ...
    children.sort(function(a, b) {
      if (a.mztwin && b.mztwin && a.mztwin == b.mztwin) return 0;
      else if (a.dztwin && b.dztwin && a.dztwin == b.dztwin) return 0;
      else if (a.mztwin || b.mztwin || a.dztwin || b.dztwin) return 1;
      return 0;
    });
  ...
  pedigree_util.setProband = function(dataset, name, is_proband) {
    $.each(dataset, function(i, p) {
      if (name === p.name) p.proband = is_proband;
      else delete p.proband;
    });
  };
  ...
  pedigree_util.getChildren = function(dataset, mother, father) {
    ...
    if (mother.sex === 'F') $.each(dataset, function(i, p) {
      if (mother.name === p.mother)
        if (!father || father.name == p.father) children.push(p);
    });
    return children;
  };
  ...
  pedigree_util.getSiblings = function(dataset, person, sex) {
    if (!person.mother || person.noparents) return [];
  ...
  pedigree_util.linkNodes = function(flattenNodes, partners) {
    var links = [];
    for (var i = 0; i < partners.length; i++) links.push({
          ...
          if (!pedigree_util.overlap(opts, root.descendants(), xmid, node.depth, [node.data.name])) {
            ...
            if (node.children.length == 2 && (node.children[0].data.hidden || node.children[1].data.hidden)) {
              if (!(node.children[0].data.hidden && node.children[1].data.hidden)) {
                ...
                if (((child1.x < child2.x && xmid < child2.x) || (child1.x > child2.x && xmid > child2.x)) && !pedigree_util.overlap(opts, root.descendants(), xmid, child1.depth, [child1.data.name])) {
                  child1.x = xmid;
                }
              }
            } else if (node.children.length == 1 && !node.children[0].data.hidden) {
              if (!pedigree_util.overlap(opts, root.descendants(), xmid, node.children[0].depth, [node.children[0].data.name])) node.children[0].x = xmid;
            } else {
  ...
  function nodesOverlap(opts, node, diff, root) {
    ...
    for (var i = 0; i < descendants.length; i++) {
      ...
      if (node.data.name !== descendant.data.name) {
        ...
        if (pedigree_util.overlap(opts, nodes, xnew, descendant.depth, descendantsNames)) return true;
      }
    }
    return false;
  }
  ...
  pedigree_util.getNodeByName = function(nodes, name) {
    for (var i = 0; i < nodes.length; i++) {
      if (nodes[i].data && name === nodes[i].data.name) return nodes[i];
      else if (name === nodes[i].name) return nodes[i];
    }
  };
  ...
  pedigree_util.proband_attr = function(opts, key, value) {
    ...
    if (value) {
      if (key in proband) {
        if (proband[key] === value) return;
        try {
          if (JSON.stringify(proband[key]) === JSON.stringify(value)) return;
        } catch (e) {}
      }
      proband[key] = value;
    } else {
      if (key in proband) delete proband[key];
      else return;
    }
  ...
  pedigree_util.print_opts = function(opts) {
    ...
    for (var i = 0; i < opts.dataset.length; i++) {
      ...
      for (key in opts.dataset[i]) {
        ...
        if (key === 'parent') person += "<span>" + key + ":" + opts.dataset[i][key].name + "; </span>";
...
(function(ptree, $, undefined) {
  ...
  ptree.build = function(options) {
    ...
    if (pedcache.nstore(opts) == -1) pedcache.add(opts);
    ...
    if (opts.DEBUG) pedigree_util.print_opts(opts);
    ...
    var node = ped.selectAll(".node").data(nodes.descendants()).enter().append("g").attr("transform", function(d, i) {
    ...
    var pienode = node.selectAll("pienode").data(function(d) { // set the disease data for the pie plot
      var ncancers = 0;
    ...
    node.append("path").filter(function(d) {
      ...
      {
        var get_bracket = function(dx, dy, indent) {
    ...
    var status = node.append('line').filter(function(d) {
      return d.data.status == 1;
    }).style("stroke", "black").attr("x1", function(d, i) {
    ...
    for (var ilab = 0; ilab < opts.labels.length; ilab++) {
      ...
      addLabel(opts, node, ".25em", -(0.7 * opts.symbol_size), calcFY1, calcFText1, 'indi_details');
    }
    var calcFY2 = function(d) {
    ...
    for (var i = 0; i < opts.diseases.length; i++) {
      ...
      addLabel(opts, node, ".25em", -(opts.symbol_size), calcFY2, calcFText2, 'indi_details');
    }
    //
    widgets.addWidgets(opts, node);
    // links between partners
    var clash_depth = {};
    partners = ped.selectAll(".partner").data(ptrLinkNodes).enter().insert("path", "g").attr("fill", "none").attr("stroke", "#000").attr("shape-rendering", "auto").attr('d', function(d, i) {
      ...
      if (clash) {
        ...
        parent_node = pedigree_util.getNodeByName(flattenNodes, parent_node_name);
      ...
      var divorce_path = "";
      if (divorced && !clash) divorce_path = "M" + (x1 + ((x2 - x1) * 0.66) + 6) + "," + (dy1 - 6) + "L" + (x1 + ((x2 - x1) * 0.66) - 6) + "," + (dy1 + 6) + "M" + (x1 + ((x2 - x1) * 0.66) + 10) + "," + (dy1 - 6) + "L" + (x1 + ((x2 - x1) * 0.66) - 2) + "," + (dy1 + 6);
    ...
    // links to children
    var links = ped.selectAll(".link").data(root.links(nodes.descendants())).enter().filter(function(d) {
      // filter unless debug is set
      return (opts.DEBUG || (d.target.data.noparents === undefined && d.source.parent !== null && !d.target.data.hidden));
    }).insert("path", "g").attr("fill", "none").attr("stroke-width", function(d, i) {
      ...
      for (var usedlen = 0; usedlen < dash_len; usedlen += 10) $.merge(dash_array, [5, 5]);
      return dash_array;
    ...
    if (probandIdx) {
      ...
      ped.append("svg:defs").append("svg:marker") // arrow head
        .attr("id", "triangle").attr("refX", 6).attr("refY", 6).attr("markerWidth", 20).attr("markerHeight", 20).attr("orient", "auto").append("path").attr("d", "M 0 0 12 6 0 12 3 6").style("fill", "black");
      ped.append("line").attr("x1", probandNode.x - opts.symbol_size).attr("y1", probandNode.y + opts.symbol_size).attr("x2", probandNode.x - opts.symbol_size / 2).attr("y2", probandNode.y + opts.symbol_size / 2).attr("stroke-width", 1).attr("stroke", "black").attr("marker-end", "url(#triangle)");
    }
    // drag and zoom
    ...
    function zoomFn() {
      ...
      if (t.k == 1) pedcache.setposition(opts, pos[0], pos[1]);
      else pedcache.setposition(opts, pos[0], pos[1], t.k);
  ...
  ptree.validate_pedigree = function(opts) {
    if (opts.validate) {
      ...
      if (unconnected.length > 0) console.warn("individuals unconnected to pedigree ", unconnected);
    }
  };
  // check if the object contains a key with a given prefix
  function prefixInObj(prefix, obj) {
    var found = false;
    if (obj) $.each(obj, function(k, n) {
  ...
  ptree.unconnected = function(dataset) {
    ...
    var change = true;
    var ii = 0;
    var checkConnections = function(idx, p) {
      if ($.inArray(p.name, connected) != -1) {
        ...
        for (var i = 0; i < ptrs.length; i++) {
          if (!pedigree_util.getNodeByName(dataset, ptrs[i]).noparents) has_parent = true;
        }
        if (has_parent) {
  ...
  function get_partners(dataset, anode) {
    ...
    for (var i = 0; i < dataset.length; i++) {
      var bnode = dataset[i];
      if (anode.name === bnode.mother && $.inArray(bnode.father, ptrs) == -1) ptrs.push(bnode.father);
      else if (anode.name === bnode.father && $.inArray(bnode.mother, ptrs) == -1) ptrs.push(bnode.mother);
    }
    return ptrs;
  }
  ...
  function group_top_level(dataset) {
    ...
    for (var i = 0; i < dataset.length; i++) {
      if (pedigree_util.getDepth(dataset, dataset[i].name) == 2) dataset[i].top_level = true;
    }
    var top_level = [];
    var top_level_seen = [];
    ...
    for (i = top_level.length; i > 0; --i) newdataset.unshift(top_level[i - 1]);
    return newdataset;
  }
  ...
  function getPx(emVal) {
    if (emVal === parseInt(emVal, 10)) // test if integer
      return emVal;
    if (emVal.indexOf("px") > -1) return emVal.replace('px', '');
    else if (emVal.indexOf("em") === -1) return emVal;
  ...
  function addLabel(opts, node, size, fx, fy, ftext, class_label) {
    node.filter(function(d) {
      ...
      }).append("text").attr("class", class_label + ' ped_label' || "ped_label").attr("x", fx).attr("y", fy)
      //.attr("dy", size)
      ...
      for (var j = 0; j < twin_types.length; j++) {
        ...
        if (dataset[i][twin_type]) {
          ...
          for (var k = 0; k < dataset.length; k++) {
            if (dataset[k][twin_type] == dataset[i][twin_type]) count++;
          }
          if (count < 2) delete dataset[i][
            [twin_type]
          ];
        }
      }
  ...
  ptree.addparents = function(opts, dataset, name) {
    ...
    if (depth == 1) {
      ...
      var midx;
      if ((!add_lhs && node_father.data.id > node_mother.data.id) || (add_lhs && node_father.data.id < node_mother.data.id)) midx = pedigree_util.getIdxByName(dataset, node.father);
      else midx = pedigree_util.getIdxByName(dataset, node.mother);
      ...
      for (i = 0; i < node.parent_node.length; i++) {
        ...
        for (j = 0; j < children.length; j++) {
          ...
          if (child) {
            ...
            var ptr;
            if (ptrs.length > 0) ptr = pedigree_util.getNodeByName(dataset, ptrs[0]);
...
(function(pedigree_form, $, undefined) {
  pedigree_form.update = function(opts) {
    ...
    $('#id_proband, #id_exclude').click(function(e) {
      ...
      if ($(this).attr("id") === 'id_proband') {
        ...
        if ($(this).is(':checked')) opts.dataset[idx].exclude = true;
        else delete opts.dataset[idx].exclude;
      }
      pedcache.add(opts);
      $("#" + opts.targetDiv).empty();
      ptree.build(opts);
    ...
    $('#id_mutation_frequencies').change(function() {
      ...
      if (pedigree_form.mutation_frequencies && this.value !== 'Custom') {
        ...
        for (var gene in mfreq) $('#id_' + gene.toLowerCase() + '_mut_frequency').val(mfreq[gene]);
      }
    });
  };
  ...
  $(document).on('fhChange', function(e, opts) {
    try {
      ...
      if (node === undefined) $('form > fieldset').prop("disabled", true);
      else $('form > fieldset').prop('disabled', false);
    } catch (err) {
      console.warn(err);
    }
  });
  pedigree_form.nodeclick = function(node) {
    ...
    if (node.sex === 'M' || node.sex === 'F') $('input[name=sex][value="' + node.sex + '"]').prop('checked', true);
    else $('input[name=sex]').prop('checked', false);
  ...
  pedigree_form.save = function(opts) {
    ...
    for (var iswitch = 0; iswitch < switches.length; iswitch++) {
      ...
      if (s.length > 0) {
        ...
        if (s.is(":checked")) person[attr] = true;
        else delete person[attr];
      }
    }
    // current sex
    ...
    else delete person.approx_diagnosis_age;
    ...
    $('#person_details input[type="checkbox"][name$="cancer"],input[type="checkbox"][name$="cancer2"]').each(function() {
      if (this.checked) person[$(this).attr('name')] = true;
      else delete person[$(this).attr('name')];
    });
    // pathology tests
...
(function(pbuttons, $, undefined) {
  ...
  function click(opts) {
    ...
    $('#fullscreen').on('click', function(e) {
      if (!document.mozFullScreen && !document.webkitFullScreen) {
        ...
        else target.webkitRequestFullScreen(Element.ALLOW_KEYBOARD_INPUT);
      } else {
  ...
  pbuttons.updateButtons = function(opts) {
    ...
    if (nstore <= current) $(id + " .fa-repeat").addClass('disabled');
    else $(id + " .fa-repeat").removeClass('disabled');
...
(function(pedcache, $, undefined) {
  ...
  function get_browser_store(opts, item) {
    if (opts.store_type === 'local') return localStorage.getItem(item);
    else return sessionStorage.getItem(item);
  }

  function set_browser_store(opts, name, item) {
    if (opts.store_type === 'local') return localStorage.setItem(name, item);
    else return sessionStorage.setItem(name, item);
  }

  function clear_browser_store(opts) {
    if (opts.store_type === 'local') return localStorage.clear();
    else return sessionStorage.clear();
  }
  pedcache.get_count = function(opts) {
    var count;
    if (has_browser_storage(opts)) count = get_browser_store(opts, get_prefix(opts) + 'COUNT');
    else count = dict_cache[get_prefix(opts) + 'COUNT'];
    if (count !== null && count !== undefined) return count;
    return 0;
  };

  function set_count(opts, count) {
    if (has_browser_storage(opts)) set_browser_store(opts, get_prefix(opts) + 'COUNT', count);
    else dict_cache[get_prefix(opts) + 'COUNT'] = count;
  }
  ...
  pedcache.current = function(opts) {
    ...
    if (current == -1) current = max_limit - 1;
    if (has_browser_storage(opts)) return JSON.parse(get_browser_store(opts, get_prefix(opts) + current));
    else if (get_arr(opts)) return JSON.parse(get_arr(opts)[current]);
  };
  ...
  pedcache.previous = function(opts, previous) {
    if (previous === undefined) previous = pedcache.get_count(opts) - 2;
    if (previous < 0) {
      var nstore = pedcache.nstore(opts);
      if (nstore < max_limit) previous = nstore - 1;
      else previous = max_limit - 1;
    }
    set_count(opts, previous + 1);
    if (has_browser_storage(opts)) return JSON.parse(get_browser_store(opts, get_prefix(opts) + previous));
    else return JSON.parse(get_arr(opts)[previous]);
  };
  pedcache.next = function(opts, next) {
    if (next === undefined) next = pedcache.get_count(opts);
    if (next >= max_limit) next = 0;
    set_count(opts, parseInt(next) + 1);
    if (has_browser_storage(opts)) return JSON.parse(get_browser_store(opts, get_prefix(opts) + next));
    else return JSON.parse(get_arr(opts)[next]);
  };
...
(function(widgets, $, undefined) {
  ...
  widgets.addWidgets = function(opts, node) {
    ...
    d3.selectAll(".persontype").on("click", function() {
      ...
      if (add_person.type === 'addsibling') ptree.addsibling(newdataset, add_person.node.datum().data, sex, false, twin_type);
      else if (add_person.type === 'addchild') ptree.addchild(newdataset, add_person.node.datum().data, (twin_type ? 'U' : sex), (twin_type ? 2 : 1), twin_type);
      else return;
      opts.dataset = newdataset;
      ptree.rebuild(opts);
      ...
      if (add_person.type === 'addsibling') {
        if (d3.select(this).classed("fa-square")) square_title.text("add brother");
    ...
    node.append("rect").filter(function(d) {
      return d.data.hidden && !opts.DEBUG ? false : true;
    }).attr("class", 'indi_rect').attr("rx", 6).attr("ry", 6).attr("x", function(d) {
    ...
    for (var key in widgets) {
      var widget = node.append("text").filter(choseNodes).attr("class", key).style("opacity", 0).attr('font-family', 'FontAwesome').attr("xx", function(d) {
        return d.x;
      }).attr("yy", function(d) {
        return d.y;
    ...
    d3.selectAll(".addchild, .addpartner, .addparents, .delete, .settings").on("click", function() {
      ...
      if (opt === 'settings') {
        ...
        var onDone = function(opts, dataset) {
  ...
  function drag_handle(opts) {
    ...
    line_drag_selection.append("line").attr("class", 'line_drag_selection').attr("stroke-width", 6).style("stroke-dasharray", ("2, 1")).attr("stroke", "black").call(d3.drag().on("start", dragstart).on("drag", drag).on("end", dragstop));
    setLineDragPosition(0, 0, 0, 0);
  ...
  function openEditDialog(opts, d) {
    ...
    // alive status = 0; dead status = 1
    table += '<tr><td colspan="2" id="id_status">' + '<label class="checkbox-inline"><input type="radio" name="status" value="0" ' + (d.data.status === 0 ? "checked" : "") + '>&thinsp;Alive</label>' + '<label class="checkbox-inline"><input type="radio" name="status" value="1" ' + (d.data.status === 1 ? "checked" : "") + '>&thinsp;Deceased</label>' + '</td></tr>';
